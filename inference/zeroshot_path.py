...
@torch.no_grad()
def zero_shot_classifier(model, model_name, classnames, templates, tokenizer=None, device=None):
    """
    ...
    for classnames_for_class in classnames:
        ...
        for classname in classnames_for_class:
            ...
            if model_name == 'KEEP':
                token_ids = tokenizer(texts, max_length=256, padding='max_length', truncation=True, return_tensors='pt') # KEEP update
                token_ids = token_ids.to(device)
                classname_embeddings = model.encode_text(token_ids)
                embeddings_for_class.append(classname_embeddings) # KEEP update
        ...
        zeroshot_prot.append(class_embedding)
    ...
    #zeroshot_prot = torch.stack(zeroshot_prot, dim=0)
    return zeroshot_weights, zeroshot_prot
