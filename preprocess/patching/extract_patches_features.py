...
def patching(WSI_object, models_dict, **kwargs):
	### Start Patch Timer
	start_time = time.time()

	# Patch
	file_path = WSI_object.create_patches_features(**kwargs, models_dict=models_dict)

	### Stop Patch Timer
	patch_time_elapsed = time.time() - start_time
	return file_path, patch_time_elapsed

def seg_and_patch(source, save_dir, mask_save_dir, stitch_save_dir, emb_save_dir, coords_save_dir, models,
	...
	for model_it in models:
		model, preprocess = load_model(model_name=model_it)
		models_dict[model_it] = (model, preprocess)
	...
	for i in range(total):
		...
		if auto_skip and os.path.isfile(os.path.join(emb_save_dir, next(iter(models_dict)), slide_id + '.npy')):
			print('{} already exist in destination location, skipped'.format(slide_id))
			df.loc[idx, 'status'] = 'already_exist'
			continue

		# Inialize WSI
		WSI_object = WholeSlideImage(slides_fp[i])
		...
		if save_mask:
			mask = WSI_object.visWSI(**current_vis_params)
			# Calcula las nuevas dimensiones (1/8 del tamaño original)
			new_width = mask.width // 8
			new_height = mask.height // 8
			# Redimensiona la imagen. Puedes usar Image.ANTIALIAS para mejorar la calidad
			mask_resized = mask.resize((new_width, new_height), resample=Image.LANCZOS)
			mask_path = os.path.join(mask_save_dir, slide_id + '.png')
		...
		if patch:
			current_patch_params.update({'patch_level': patch_level, 'patch_size': patch_size, 'step_size': step_size,
										 'emb_save_dir': emb_save_dir, 'coords_save_dir': coords_save_dir, 'custom_downsample': custom_downsample})
			file_path, patch_time_elapsed = patching(WSI_object = WSI_object, models_dict = models_dict, **current_patch_params)
...
parser.add_argument('--save_dir', type = str,
					help='directory to save processed memory_bank')
parser.add_argument('--preset', default=None, type=str,
					help='predefined profile of default segmentation and filter parameters (.csv)')
parser.add_argument('--patch_level', type=int, default=0, 
					help='downsample level at which to patch')
parser.add_argument('--custom_downsample', type= int, choices=[1,2,4], default=1,
					help='custom downscale when native downsample is not available (only tested w/ 2x downscale)')
parser.add_argument('--process_list',  type = str, default=None,
					help='name of list of images to process with parameters (.csv)')
parser.add_argument('--models',  type = str, default="KEEP,CONCH",
					help='Patch-level foundation models for feature extraction. Separated by commas')
...
if __name__ == '__main__':
	...
	print('emb_save_dir: ', emb_save_dir)
	print('coords_save_dir: ', coords_save_dir)
	...
	directories = {'source': args.source, 
				   ...
				   'coords_save_dir': coords_save_dir,
	...
	# Support of patch-level feature extraction
	models = args.models.split(",") # List of histopathology FM
	for model_it in models:
		os.makedirs((os.path.join(emb_save_dir, model_it)), exist_ok=True) # Create directories
	...
	seg_times, patch_times = seg_and_patch(**directories, **parameters, models=models,
