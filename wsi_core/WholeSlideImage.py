...
class WholeSlideImage(object):
    ...
    def create_patches_features(self, emb_save_dir, coords_save_dir, models_dict, patch_level=0, patch_size=256, step_size=256, **kwargs):
        print("Extracting features of patches for: ", self.name, "...",)
        embeddings, coords = {}, []
        for model_it in models_dict.keys():
            embeddings[model_it] = []
        contours = self.contours_tissue
        for idx, cont in enumerate(contours):
            patch_gen = self._getPatchGenerator(cont, idx, patch_level, emb_save_dir, patch_size, step_size, **kwargs)
            # for patch in patch_gen:
            #     coords.append([patch['x'], patch['y']])
            #     patch_PIL = patch['patch_PIL']
            ...
            for patch in patch_gen:
                ...
                for model_it in models_dict.keys():
                    model, preprocess = models_dict[model_it]
                    patch_t = preprocess(patch_PIL).unsqueeze(dim=0).to(device)
                    with torch.inference_mode() and torch.no_grad():
                        x = extract_embedding(patch_t, model_it, model)
                    embeddings[model_it].append(x)

        if not contours or not embeddings[next(iter(models_dict))]: # Support for no detected tissue/patches
            return
        for model_it in models_dict.keys():
            embeddings_model = np.stack(embeddings[model_it])
            np.save(os.path.join(emb_save_dir, model_it, patch['name']), embeddings_model)
        file_save_coords = os.path.join(coords_save_dir, patch['name'])
        np.save(file_save_coords, np.stack(coords))
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if custom_downsample > 1:
            ...
            patch_size = target_patch_size * custom_downsample
